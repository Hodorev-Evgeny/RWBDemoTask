...
func (t *TransportStopList) DeleteStopList(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	err = t.serviceStopList.DeleteStopList(ctx, key)
	if err != nil {
		responseHandler.ErrorResponse(err, "error deleting stop list")
		return
	}
